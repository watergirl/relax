...
#import "TuWanListViewController1.h"
#import "TuWanListCell1.h"
#import "TuWanViewModel1.h"
#import "TuWanImageCell1.h"
#import "iCarousel.h"
#import "TuWanHtmlViewController1.h"
#import "TuWanPicViewController1.h"

@interface TuWanListViewController1 ()<iCarouselDelegate,iCarouselDataSource>
@property(nonatomic,strong)TuWanViewModel1 *tuwanVM;
@end

@implementation TuWanListViewController1
...
-(TuWanViewModel1 *)tuwanVM
{
    if (!_tuwanVM) {
        _tuwanVM = [[TuWanViewModel1 alloc] initWithType:_infoType.integerValue];
    }
    return _tuwanVM;
}
- (void)viewDidLoad {
    [super viewDidLoad];
    [self.tableView registerClass:[TuWanListCell1 class] forCellReuseIdentifier:@"ListCell"];
    [self.tableView registerClass:[TuWanImageCell1 class] forCellReuseIdentifier:@"ImageCell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([self.tuwanVM containImages:indexPath.row]) {
        TuWanImageCell1 *cell = [tableView dequeueReusableCellWithIdentifier:@"ImageCell"];
    ...
    TuWanListCell1 *cell = [tableView dequeueReusableCellWithIdentifier:@"ListCell" forIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.tuwanVM isHtmlInListForRow:indexPath.row]) {
        TuWanHtmlViewController1 *vc=[[TuWanHtmlViewController1 alloc] initWithURL:[self.tuwanVM detailURLForRowInList:indexPath.row]];
        [self.navigationController pushViewController:vc animated:YES];
    }
    
    if ([self.tuwanVM isPicInListForRow:indexPath.row]) {
        TuWanPicViewController1 *vc =[[TuWanPicViewController1 alloc] initWithAid:[self.tuwanVM aidInListForRow:indexPath.row]];
        [self.navigationController pushViewController:vc animated:YES];
    }
...
- (void)carousel:(iCarousel *)carousel didSelectItemAtIndex:(NSInteger)index{
    if ([self.tuwanVM isHtmlInIndexPicForRow:index]) {
        TuWanHtmlViewController1 *vc=[[TuWanHtmlViewController1 alloc] initWithURL:[self.tuwanVM detailURLForRowInIndexPic:index]];
        [self.navigationController pushViewController:vc animated:YES];
    }
    if ([self.tuwanVM isPicInIndexPicForRow:index]) {
        TuWanPicViewController1 *vc=[[TuWanPicViewController1 alloc] initWithAid:[self.tuwanVM aidInIndexPicForRow:index]];
