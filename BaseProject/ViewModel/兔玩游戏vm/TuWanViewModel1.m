...
@implementation TuWanViewModel1
...
- (void)getDataFromNetCompleteHandle:(CompletionHandle)completionHandle{
    self.dataTask = [TuWanNetManager1 getTuWanInfoWithType:_type start:_start completionHandle:^(TuWanModel1 *model, NSError *error) {
        if (_start == 0) {
...
- (TuWanDataIndexpicModel1 *)modelForArr:(NSArray *)arr row:(NSInteger)row{
    return arr[row];
}
...
- (NSArray *)iconURLSForRowInList:(NSInteger)row{
    ...
    for (int i = 0; i < arr.count; i ++) {
        TuWanDataIndexpicShowitemModel1 *model= arr[i];
